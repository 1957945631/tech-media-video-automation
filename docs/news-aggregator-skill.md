...
1. 先运行 `npm run news:aggregate:assets -- --date YYYY-MM-DD`。
2. Skill 输出 `data/assets/YYYY-MM-DD/news-aggregator-research.json`。
3. 再运行 `npm run capture:daily-assets -- YYYY-MM-DD`。
4. `capture_daily_assets` 会自动读取 skill 输出，并把候选链接并入素材 source pools。
