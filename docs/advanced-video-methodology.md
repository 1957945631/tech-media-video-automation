...
- `concept`：抽象概念位。成片不得用泛化科技图糊弄观众，应转成真实素材、结构拆解、Remotion 机制动画或关键判断卡。
- `motion`：机制、流程、依赖关系、系统运行和风险扩散。通常由 `remotion_motion_clip` 在最终视频中内嵌渲染。
...
- 机制素材：流程图、因果图、技术路径、架构图和 Remotion 语义动画。
- 补充素材：真实 b-roll、产品界面、产业现场或信息卡。

正式视频应混合使用官方截图、产品 UI、真实场景、结构拆解、Remotion 语义动画和关键词观点卡。不要把内部工具名或素材状态放到观众可见内容里。
...
- 每 20 秒至少出现一次 `diagram`、`motion` 或 `keyword`。
- Logo 只能作为过渡，不能撑满超过 3 秒。
- 证据截图可以有来源标签、裁切、下划线、局部放大或人工确认的宽区域强调。
- 禁止固定坐标红框、固定箭头和无法确认位置的标注。
- 泛化概念画面不能连续超过 12 秒，后面必须接现实素材、证据素材或明确的 Remotion 机制动画。
- 任何 10 秒内都要有至少一次信息变化。
...
这套规则优先约束素材获取和 visual beat 规划；Remotion 组件应继续向 `EvidenceCard`、`BrollCard`、`ConceptCard`、`MotionClip`、`DiagramCard`、`KeywordPunch` 的组件库方向演进。
